...
# pin0's big pad), plus /imu (accelerometer x y z), /btn (buttons A B),
# /display/text <string> (scroll text), and /s/q (exit to the REPL).
...
def run():
    ...
    def handle(addr, args, out):
        ...
        if addr == '/s/m':
            ...
            # ADDRESSES.md says g; the micro:bit reports milli-g.
    ...
    gc.collect()
    def enq():
        # The capability bundle of ADDRESSES.md: same addresses as every board.
        return [message('/enq', '{{ID}}Oscuino'), message('/enq/imu', 3),
