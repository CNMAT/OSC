...
| EC Buying STM32F407VET6 | 22/22 | 11/11, int=4 long=4 ll=8 double=8 | 7/7 | bench 2026-08-24: stock 192 B CDC queue drops with `seqErrs`+`crcErrs` from ~220 B bursts (ring ceiling ~200 B, compound lossy); queue at 4096 via `build_opt.h`: gate + in 50 ×3 (12,500 f/s) + out 200 ×3 + compound ×3 + 1100 B ring **all clean** — the A/B that convicts the queue. The FIRST big-queue A/B was invalid: the define went to the C++ compiler only and `cdc_queue.c` is C; the tell is the RAM report, +3,904 B = (64−3)×64 when the flag really lands |
