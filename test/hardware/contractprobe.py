...
ACTUATORS = {
    ...
    'led':     dict(addr='/s/l', args=(0,), reply='/s/l', note='turns it off'),
}
...
def main():
    ...
    # /s/l and /s/v used to be probed here as though every board had them.
    # They are announced capabilities now (ADDRESSES.md), so the greeting
    # decides and the sweep below tests them -- no guessing from silence.
    if 'led' in enq:
        for v in (1, 0):
            r = ask([('/s/l', (v,))])
