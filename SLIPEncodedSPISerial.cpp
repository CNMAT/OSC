...
//use StreamSPI
SLIPEncodedSPISerial::SLIPEncodedSPISerial(StreamSPI &s){
	serial = &s;
	rstate = CHAR;
}
...
/*
 SERIAL METHODS
 */
bool SLIPEncodedSPISerial::endofPacket()
{
	if(rstate == SECONDEOT)
	{
		rstate = CHAR; 
...
int SLIPEncodedSPISerial::available(){
back:
...
//encode SLIP
 void SLIPEncodedSPISerial::write(uint8_t b){
	if(b == eot){ 
...
void SLIPEncodedSPISerial::write(const uint8_t *buffer, size_t size) {  while(size--) write(*buffer++); }
#else
//encode SLIP
size_t SLIPEncodedSPISerial::write(uint8_t b){
	if(b == eot){ 
		serial->write(slipesc);
		return serial->write(slipescend); 
	} else if(b==slipesc) {  
...
size_t SLIPEncodedSPISerial::write(const uint8_t *buffer, size_t size) { size_t result; while(size--) result = write(*buffer++); return result; }

#endif

void SLIPEncodedSPISerial::begin(){
	serial->begin();
}
//SLIP specific method which begins a transmitted packet
void SLIPEncodedSPISerial::beginPacket() { 	serial->write(eot); }

//signify the end of the packet with an EOT
void SLIPEncodedSPISerial::endPacket(){
	serial->write(eot);

}

void SLIPEncodedSPISerial::flush(){
	serial->flush();
}
