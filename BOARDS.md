...
The stack, not the board, is what determines transport behaviour. Ten
...
| WCH **CH343 bridged UART** (Waveshare ESP32-P4-POE-ETH) | clean | clean | the second non-USB transport here, and the well-behaved one: clean on every rung including the lazy-reader ring that the UNO R4's bridge fails. Same shape of path — the MCU speaks UART to a bridge chip that terminates the USB side — so the bridge, not the MCU, is what the row characterises |
...
| Adafruit QT Py **ESP32-S2** | ESP32-S2 (single core, 240 MHz), 4 MB flash + 2 MB PSRAM, MAC 58:cf:79:a5:e5:02 | echo 22/22 · widths 11/11 (int=4 long=4 ll=8) · gate CLEAN · in 50 one-write ×3, out 200 ×3, compound ×3, ring 20 ×3 — all CLEAN; 2026-09-06 | Stock FQBN `esp32:esp32:adafruit_qtpy_esp32s2`, which sets `cdc_on_boot=1` itself. Clean on every rung and needs no new detection rung — the S2's single core does not change the USB picture. **It moves ports under you:** during this bringup it alternated between `usbmodemNNNN01` (its ROM download port) and its app port twice, and an upload aimed at the stale name failed with "Check if the port is correct". Re-list immediately before every upload; do not reuse a port name across two commands. The S3 sibling's BOOT+RESET first-flash gesture was NOT needed here — a plain upload worked. |
| Waveshare **ESP32-P4-POE-ETH** | **ESP32-P4** (dual core RISC-V + LP core, 400 MHz), MAC 30:ed:a0:ea:52:4c, reached over a WCH CH343 bridge (`1a86:55d3`) | echo 22/22 ×3 · widths 11/11 (int=4 long=4 ll=8) · gate CLEAN · in 50 one-write ×3, out 200 ×3, compound ×3, ring 20 ×3 — all CLEAN; 2026-09-06 | **First ESP32-P4 in this table, and a new chip family** — and it needed no new rung in `SLIPEncodedSerial.h`. The board has no native-USB path to the host: `waveshare_p4_poe_eth.build.cdc_on_boot=0` and a CH343 UART bridge carries the console, so `Serial` is a UART and the board belongs with the UNO R4 and the MG24 in the **bridged-UART** family rather than any USB-stack row. FQBN `esp32:esp32:waveshare_p4_poe_eth`. It arrived stuck in a task-watchdog panic loop on its factory firmware (`MHARTID`/`MTVAL 0xdeadc0de` repeating on the console); flashing replaced that. **First echo run showed 1 mismatch of 22, and it was the instrument:** the panic output was still draining from the buffer on the first read after flashing. Three consecutive re-runs were clean, which is why the row says 22/22 ×3 — a single unrepeated mismatch is not a measurement. |
| LilyGO **T-FPGA** (module H622, M.2 card for the T-FPGA shield carrier) | ESP32-S3 (QFN56) rev v0.2, dual core + LP core, 240 MHz, 8 MB *embedded* PSRAM (AP_3v3, octal), 16 MB Winbond flash (`ef`/`4018`), eFuse says **quad** flash at 3.3 V, 40 MHz crystal, MAC a0:f2:62:e1:16:bc; alongside a Gowin **GW1NSR-LV4CQN48PC6/I5** FPGA and an **AXP2101** PMU | echo 22/22 · widths 11/11 (int=4 long=4 ll=8 double=8) · bench verify ×3, in 50 -1 ×3, out 200 ×3, ring 20 ×3, compound ×3 — all CLEAN; 2026-09-06 | **Identified from its own factory firmware, not from a guess.** The board enumerates as a bare `303a:1001` "USB JTAG/serial debug unit" like every other native-USB ESP32, so nothing on the bus distinguishes it. Before overwriting anything the 16 MB flash was read out to `~/Documents/Arduino/firmware-backups/esp32s3-a0f262e116bc-factory-16MB.bin` (768 KB of content, the rest erased) — and its strings name the board outright: `Hello T-FPGA-CORE`, built from `/Users/lewis/.platformio/` (LILYGO's own engineer) against `XPowersLib`'s `XPowersAXP2101`, with `Search FPGA Devices ....` / `Find FPGA I2C Devices` / `Now blink FPGA Led ...`. **Reading a board's existing flash is the cheapest identification there is, and it is also the backup you want before you flash over it.** The MAC OUI `a0:f2:62` is Espressif's own block, so it names the silicon and says nothing about the carrier — *the chip is not the board*, again.\n\nFQBN, from the vendor's committed Arduino settings table (<https://github.com/Xinyuan-LilyGO/T-FPGA>):\n\n`esp32:esp32:esp32s3:FlashSize=16M,FlashMode=qio,PSRAM=opi,CDCOnBoot=cdc,USBMode=hwcdc,PartitionScheme=huge_app`\n\n**That table contradicts itself and cannot be followed literally**: it asks for *USB DFU On Boot = Enabled* and *USB Mode = Hardware CDC and JTAG* together, but the core marks `DFUOnBoot=dfu` as "Requires USB-OTG Mode", which is the other `USBMode`. The board enumerates as USB-Serial/JTAG, so `USBMode=hwcdc` is the true one and DFU stays default. Note also `PartitionScheme=huge_app`, not the `app3M_fat9M_16MB` used on the T-Encoder-Pro; the first build here used the latter, worked, and was rebuilt on the vendor's set so the recorded FQBN is the one actually measured.\n\n**PINS** (vendor `example/ESP32-S3/*/pins_config.h`): BTN=IO0, LED=IO46, I2C SDA=IO38 SCL=IO39, PMU IRQ=IO40, and six lines to the FPGA — CS=IO1, SCK=IO2, D0=IO3, D1=IO5, D2=IO6, D3=IO4. The FPGA also answers as an **I2C slave** on the same bus as the PMU; the vendor ships both an `i2c_blink` and an `spi_blink` example, plus the Gowin project that makes the FPGA answer.\n\n**THE FPGA HAS NO POWER UNTIL THE ESP32-S3 GIVES IT SOME, and the rails are software-set.** From `turn_on_power.ino`: `DC4` is the **FPGA core voltage** (1200 mV), `ALDO1..ALDO4` are the four I/O **bank** voltages (3300, 3300, 2500, 1800 mV) and `BLDO1`/`BLDO2` are the shield-side rails (1800 mV) — the carrier's I/O voltage, which is what makes the M.2-plus-baseboard arrangement work. The vendor's own comment on DC4 and the BLDOs is *"Careful review of the manual is required before modification"*, so those numbers are not to be improvised. Any OSC demo on this board must bring the AXP2101 up first — the same shape of hazard as the M5Capsule's GPIO46 power hold, and a sketch that skips it will find a dead FPGA and blame the FPGA. **The transport results above were taken with the PMU unconfigured**, which is legitimate — `OscEcho`, `IntWidths` and `OscBench` drive no pins and need no FPGA — but it means nothing here has yet powered or spoken to the FPGA. That is Phase 4 and is not done. |
| LilyGO **T-Encoder-Pro** | ESP32-S3-R8 (dual core, 240 MHz), 16 MB Winbond flash (`ef`/`4018`) + 8 MB OPI PSRAM, MAC 24:58:7c:d3:62:a4 | echo 22/22 · widths 11/11 (int=4 long=4 ll=8) · gate CLEAN · in 50 one-write ×3, out 200 ×3, compound ×3, ring 20 ×3 — all CLEAN; 2026-09-06 | **No board definition exists in any installed core**, so it is built as `esp32:esp32:esp32s3` with the four options the vendor's README specifies — omit them and you get the wrong PSRAM mode and a partition table that does not fit:\n\n`esp32:esp32:esp32s3:FlashSize=16M,PSRAM=opi,CDCOnBoot=cdc,PartitionScheme=app3M_fat9M_16MB`\n\nVendor source: <https://github.com/Xinyuan-LilyGO/T-Encoder-Pro>, and the authority is its `libraries/Mylibrary/pin_config.h`, not the README: knob phases A=IO1 B=IO2, knob key=IO0, buzzer=IO17, I2C SDA=IO5 SCL=IO6, touch INT=IO9 RST=IO8, screen CS=IO10 SCLK=IO12 RST=IO4 EN=IO3, QSPI data IO11/IO13/IO7/IO14, panel 390x390. **The knob is verified by motion** (2026-09-06: turned by hand, 216 samples, position swept 0..24, first motion counting UP, so clockwise increases — taking the instruction at its word). **TWO REVISIONS SHIP UNDER THIS NAME, and this unit is not the one the vendor's default selects.** `pin_config.h` is a build-time switch between DXQ120MYB2416A (SH8601 panel, CHSC5816 touch at 0x2E) and TFD12MASBCTB4_V0_07 (CO5300, CST816 at 0x15), and the vendor's headline `examples/GFX/GFX.ino` builds SH8601 under a `// DXQ120MYB2416A` comment while `pin_config.h` has that macro commented **out** — their own example and their own default disagree. Since the two touch controllers answer at different addresses, the sketch sweeps the bus in `setup()` and lets the address choose the pair (`Arduino_OLED` is the common base of both panels, `TouchDrvInterface` of both touch drivers), reporting the result on `/diag`. Measured here across three hard resets, identical each time: `i2c: 0x2E rev:chsc5816/sh8601` — **the SH8601/CHSC5816 revision**. Now wired: `/enq/enc`, `/enq/btn`, `/enq/display 390 390`, `/enq/touch 390 390`, `/enq/diag`; contract probe 14 passed, 0 failed. Libraries are "GFX Library for Arduino" ≥ 1.6.3 and SensorLib, both from the Arduino index — the vendor's bundled `Arduino_GFX-1.5.0` is a **fork** (upstream 1.5.0 has no SH8601 at all; it arrives at 1.6.3) whose `Display_Brightness`/`SetContrast` upstream spells `setBrightness`/`setContrast`, and whose constructor still carries a `bool ips` that upstream dropped. Still not wired: the IO17 buzzer. **TWO APPARATUS FAULTS, both mine, both now fixed in the sketch:** reading the touch controller without gating on TOUCH_INT returns whatever was last latched — here a phantom finger at (182,191) that never moved across 891 samples and that the contract probe correctly saw as `/touch` streamed forever; and sweeping the bus *before* pulsing TOUCH_RST asks a question a part held in reset cannot answer, which made 0x2E appear on one boot and vanish on the next. **The key is announced but still unseen, and the schematic says why the search was misdirected**: encoder U4 has three electrical terminals (A, C, B) with both shell pads unconnected, so the press is *not* an encoder terminal — it is a separate tact switch SW1, one pin to GND and the other through R8 100K to net KEY, landing on GPIO0 (four vendor sources agree). No vendor document says what mechanically closes SW1; the only source that speaks to it is a review on LilyGO's own product page saying the button is pressed by clicking the **screen**, and only "in the very center". Three windows here pressed the knob body and saw nothing. GPIO0 is also BOOT, so it must never be sampled as a boot-time gate. **This board has no IMU, RTC, PMU or haptic engine** — this row claimed all four until 2026-09-06; they came from directory names inside the vendor's generic `Arduino_DriveBus` library, not from this board, and neither the corrected schematic's parts list nor the README's module list contains them. The first bringup here announced only `/enq/btn` on a board named after its encoder, which is what BRINGUP.md's *The documentation is a work list* now exists to stop. **Identification note:** it was first guessed to be a second M5Capsule. It is not — `flash-id` reads 16 MB Winbond plus 8 MB PSRAM here against the Capsule's 8 MB GigaDevice and no PSRAM, on a different port and MAC. The disagreement between guess and measurement is what prompted asking again. |
