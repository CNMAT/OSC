...
void SLIPEncodedSerial::write(const uint8_t *buffer, size_t size) {  serial->write(buffer, size); }
#else
//encode SLIP
...
size_t SLIPEncodedSerial::write(const uint8_t *buffer, size_t size) { return serial->write(buffer, size); }
